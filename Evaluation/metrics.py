...
class _Global_Item_Distribution_Counter(_Metrics_Object):
    ...
    def merge_with_other(self, other_metric_object):
        ...
        ), "{}: attempting to merge with a metric object of different type".format(self.__class__)
...
class Coverage_User_Correct(_Metrics_Object):
    ...
    def merge_with_other(self, other_metric_object):
        assert (
            other_metric_object is Coverage_User
        ), "Coverage_User_Correct: attempting to merge with a metric object of different type"
...
class TestMAP(unittest.TestCase):
    def runTest(self):
        ...
        self.assertTrue(np.allclose(map(ranked_list_1, pos_items), (1.0 / 2 + 2.0 / 4 + 3.0 / 5) / 4))
        self.assertTrue(np.allclose(map(ranked_list_2, pos_items), 1.0))
        self.assertTrue(np.allclose(map(ranked_list_3, pos_items), 0.0))
        self.assertTrue(np.allclose(map(ranked_list_4, pos_items), (1.0 / 7 + 2.0 / 8 + 3.0 / 9 + 4.0 / 10) / 4))
        self.assertTrue(np.allclose(map(ranked_list_5, pos_items), (1.0 + 2.0 / 7 + 3.0 / 8 + 4.0 / 9) / 4))

        thresholds = [1, 2, 3, 4, 5]
        values = [0.0, 1.0 / 2 / 2, 1.0 / 2 / 3, (1.0 / 2 + 2.0 / 4) / 4, (1.0 / 2 + 2.0 / 4 + 3.0 / 5) / 4]
        for at, val in zip(thresholds, values):
...
class TestNDCG(unittest.TestCase):
    def runTest(self):
        ...
        idcg = (2**5 - 1) / np.log(2) + (2**4 - 1) / np.log(3) + (2**3 - 1) / np.log(4) + (2**2 - 1) / np.log(5)
        self.assertTrue(np.allclose(dcg(np.sort(pos_relevances)[::-1]), idcg))
        self.assertTrue(
            np.allclose(
                ndcg(ranked_list_1, pos_items, pos_relevances),
                ((2**5 - 1) / np.log(3) + (2**4 - 1) / np.log(5) + (2**3 - 1) / np.log(6)) / idcg,
            )
        )
        self.assertTrue(
            np.allclose(
