...
class SearchAbstractClass(object):
    ...
    def _remove_intermediate_cases(self, cases_to_remove_list):
        ...
        self.dataIO.save_data(
            data_dict_to_save=self.metadata_dict.copy(), file_name=self.output_file_name_root + "_metadata"
        )
    ...
    def _evaluate_on_validation(
        self, current_fit_hyperparameters, was_already_evaluated_flag, was_already_evaluated_index
    ):
        """
    ...
    def _evaluate_on_test_with_data_last(self):
        ...
        ), "{}: Best hyperparameters not available, the search might have failed.".format(self.ALGORITHM_NAME)
    ...
    def _objective_function(self, current_fit_hyperparameters_dict):
        ...
        try:
            ...
            current_fit_hyperparameters_dict = (
                self.metadata_dict["hyperparameters_df"].loc[self.model_counter].to_dict()
            )
            ...
            if self.save_model in ["all"] and not was_already_evaluated_flag:
                ...
                recommender_instance.save_model(
                    ...
                    file_name=self.output_file_name_root + "_model_{}".format(self.model_counter),
                )
            ...
            if new_best_config_found:
                ...
                self._write_log(
                    "{}: Config {} {}. Config: {} - results: {}\n".format(
                        ...
                        "is suboptimal"
                        if not was_already_evaluated_flag
                        else "was already explored at index {}".format(was_already_evaluated_index),
                        current_fit_hyperparameters_dict,
                        get_result_string_prettyprint(result_series, n_decimals=7),
                    )
                )
            ...
            self._write_log(
                "{}: Config {} Exception. Config: {} - Exception: {}\n".format(
                    self.ALGORITHM_NAME, self.model_counter, current_fit_hyperparameters_dict, traceback_string
                )
            )
