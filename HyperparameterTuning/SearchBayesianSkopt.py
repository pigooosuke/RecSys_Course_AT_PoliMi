...
def _extend_dataframe(initial_dataframe, new_rows):
    ...
    if isinstance(initial_dataframe.index, pd.MultiIndex):
        ...
        cases_cutoff_multiindex = pd.MultiIndex.from_product(
            [range(first_level_index.max() + 1, first_level_index.max() + 1 + new_rows), second_level_index]
        )
        ...
        new_df = pd.DataFrame(
            columns=initial_dataframe.columns, index=range(len(initial_dataframe), len(initial_dataframe) + new_rows)
        )
...
class TimeoutError(Exception):
    def __init__(self, max_total_time_seconds, current_total_time):
        ...
        message = (
            "Total training and evaluation time is {:.2f} {}, exceeding the maximum threshold of {:.2f} {}".format(
...
class SearchBayesianSkopt(SearchAbstractClass):
    ...
    def __init__(self, recommender_class, evaluator_validation=None, evaluator_test=None, verbose=True):
        ...
        super(SearchBayesianSkopt, self).__init__(
            recommender_class, evaluator_validation=evaluator_validation, evaluator_test=evaluator_test, verbose=verbose
        )
    ...
    def _resume_from_saved(self):
        ...
        for index in range(self.model_counter):
            ...
            is_validation_valid = (
                ...
                and result_on_validation_df[self.metric_to_optimize].notna()[index].any()
            )
            assert (
        ...
        if result_on_validation_df is not None:
            ...
            result_on_validation_list_input = (
                -result_on_validation_df[self.metric_to_optimize].loc[:, self.cutoff_to_optimize]
            ).to_list()
        else:
    ...
    def _objective_function_list_input(self, current_fit_hyperparameters_list_of_values):
        ...
        estimated_last_time = (
            ...
            .loc[self.metadata_dict["hyperparameters_best_index"]][["train", "validation"]]
            .sum()
