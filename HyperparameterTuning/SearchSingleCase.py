...
class SearchSingleCase(SearchAbstractClass):
    ...
    def _evaluate_on_validation(
        ...
        if self.evaluator_validation is not None:
            return super(SearchSingleCase, self)._evaluate_on_validation(
                current_fit_hyperparameters, was_already_evaluated_flag, was_already_evaluated_index
            )
    ...
    def search(
        ...
        ), "{}: fit_hyperparameters_values must contain a dictionary".format(self.ALGORITHM_NAME)
