...
class DataSplitter_leave_k_out(_DataSplitter):
    ...
    def __init__(
        ...
        super(DataSplitter_leave_k_out, self).__init__(
    ...
    def _split_data_from_original_dataset(self, save_folder_path):
        ...
        if not self.allow_cold_users:
            ...
            self._print(
                "Removing {} ({:4.1f}%) of {} users because they have less than the {} interactions required for {} splits ({} for test [and validation if requested] +1 for train)".format(
                    URM.shape[0] - user_to_preserve.sum(),
    ...
    def _verify_data_consistency(self):
        ...
        assert all(URM_name in self.SPLIT_URM_DICT for URM_name in URM_to_load_list), (
        ...
        assert np.all(user_interactions == self.k_out_value), (
            print_preamble
            + "Not all users have the desired number of interactions in URM_test, {} users out of {}".format(
                (user_interactions != self.k_out_value).sum(), n_users
            )
        )

        if self.use_validation_set:
            assert self.SPLIT_URM_DICT["URM_validation"].nnz != 0, (
                print_preamble + "Number of interactions in URM Validation is 0"
            )
            ...
            assert np.all(user_interactions == self.k_out_value), (
                print_preamble
                + "Not all users have the desired number of interactions in URM_validation, {} users out of {}".format(
                    (user_interactions != self.k_out_value).sum(), n_users
                )
            )
