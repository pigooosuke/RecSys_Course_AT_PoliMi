...
def split_big_CSR_in_columns(sparse_matrix_to_split, num_split=2):
    ...
    assert (
        num_split >= 1 and num_split <= sparse_matrix_to_split.shape[1]
    ), "split_big_CSR_in_columns: num_split parameter not valid, value must be between 1 and {}, provided was {}".format(
...
def remove_Dataframe_duplicates(
    dataframe, unique_values_in_columns=["UserID", "ItemID"], keep_highest_value_in_col="timestamp"
):
    """
...
def load_CSV_into_Dataframe(
    ...
    if contains_duplicates_flag:
        if remove_duplicates:
            df_original = remove_Dataframe_duplicates(
                df_original,
...
def remove_features(ICM, min_occurrence=5, max_percentage_occurrence=0.30, reconcile_mapper=None):
    ...
    print(
        "RemoveFeatures: removed {} features with less then {} occurrences, removed {} features with more than {} occurrencies".format(
            sum(numOccurrences < min_occurrence),
            min_occurrence,
...
def reconcile_mapper_with_removed_tokens(key_to_value_dict, values_to_remove):
    ...
    assert len(set(key_to_value_dict.values())) == len(
        key_to_value_dict
    ), "mapper_dict values do not have a 1-to-1 correspondance with the key"
...
def urllretrieve_reporthook(count, block_size, total_size):
    ...
    sys.stdout.write(
        "\rDataReader: Downloaded {:.2f}%, {:.2f} MB, {:.0f} KB/s, {:.0f} seconds passed".format(
