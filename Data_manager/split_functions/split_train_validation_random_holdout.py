...
def split_train_in_two_percentage_user_wise(URM_train, train_percentage=0.1, verbose=False):
    ...
    assert (
        train_percentage >= 0.0 and train_percentage <= 1.0
    ), "train_percentage must be a value between 0.0 and 1.0, provided was '{}'".format(train_percentage)
...
def split_train_in_two_percentage_global_sample(URM_all, train_percentage=0.1):
    ...
    URM_train_builder.add_data_lists(
        URM_train.row[indices_for_train], URM_train.col[indices_for_train], URM_train.data[indices_for_train]
    )
