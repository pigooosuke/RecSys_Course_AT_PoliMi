...
class DatasetMapperManager(object):
    ...
    def generate_Dataset(self, dataset_name, is_implicit):
        ...
        for URM_name, URM_dataframe in self.URM_DICT.items():
            ...
            URM_sparse_builder.add_data_lists(
                URM_dataframe["UserID"].values, URM_dataframe["ItemID"].values, URM_dataframe["Data"].values
            )
        ...
        for ICM_name, ICM_dataframe in self.ICM_DICT.items():
            ...
            ICM_sparse_builder.add_data_lists(
                ICM_dataframe["ItemID"].values, ICM_dataframe["FeatureID"].values, ICM_dataframe["Data"].values
            )
        ...
        for UCM_name, UCM_dataframe in self.UCM_DICT.items():
            ...
            UCM_sparse_builder.add_data_lists(
                UCM_dataframe["UserID"].values, UCM_dataframe["FeatureID"].values, UCM_dataframe["Data"].values
            )
    ...
    def add_URM(self, URM_dataframe: pd.DataFrame, URM_name):
        ...
        assert all(
            is_string_dtype(URM_dataframe[ID_column]) for ID_column in ["UserID", "ItemID"]
        ), "ID columns must be strings"
        assert not self.__Dataset_finalized, "Dataset mappers have already been generated, adding new data is forbidden"
    ...
    def add_ICM(self, ICM_dataframe: pd.DataFrame, ICM_name):
        ...
        assert set(["ItemID", "FeatureID", "Data"]).issubset(
            set(ICM_dataframe.columns)
        ), "Dataframe columns not correct"
        assert all(
            is_string_dtype(ICM_dataframe[ID_column]) for ID_column in ["ItemID", "FeatureID"]
        ), "ID columns must be strings"
        assert not self.__Dataset_finalized, "Dataset mappers have already been generated, adding new data is forbidden"
    ...
    def add_UCM(self, UCM_dataframe: pd.DataFrame, UCM_name):
        ...
        assert set(["UserID", "FeatureID", "Data"]).issubset(
            set(UCM_dataframe.columns)
        ), "Dataframe columns not correct"
        assert all(
            is_string_dtype(UCM_dataframe[ID_column]) for ID_column in ["UserID", "FeatureID"]
        ), "ID columns must be strings"
        assert not self.__Dataset_finalized, "Dataset mappers have already been generated, adding new data is forbidden"
