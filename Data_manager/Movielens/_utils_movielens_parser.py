...
def _loadICM_genres_years(genres_path, header=True, separator=",", genresSeparator="|"):
    ...
    ICM_genres_dataframe = pd.read_csv(
        filepath_or_buffer=genres_path, sep=separator, header=header, dtype={0: str, 1: str, 2: str}, engine="python"
    )
    ...
    ICM_genres_dataframe = pd.DataFrame(
        ICM_genres_dataframe["GenreList"].str.split(genresSeparator).tolist(), index=ICM_genres_dataframe["ItemID"]
    ).stack()
...
def _loadUCM(UCM_path, header=True, separator=","):
    ...
    ICM_builder = IncrementalSparseMatrix_FilterIDs(
        preinitialized_col_mapper=None, on_new_col="add", preinitialized_row_mapper=None, on_new_row="add"
    )
