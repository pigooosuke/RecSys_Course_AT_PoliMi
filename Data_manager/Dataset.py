...
from Data_manager.DataReader_utils import compute_density, reconcile_mapper_with_removed_tokens, remove_features
...
class Dataset(object):
    ...
    def _assert_is_initialized(self):
        assert (
            self.AVAILABLE_URM is not None
        ), "DataReader {}: Unable to load data split. The split has not been generated yet, call the load_data function to do so.".format(
    ...
    def verify_data_consistency(self):
        ...
        if self.is_implicit():
            assert np.all(URM_all.data == 1.0), (
                print_preamble + "The DataReader is stated to be implicit but the main URM is not"
            )

        assert_URM_ICM_mapper_consistency(
            URM_DICT=self.AVAILABLE_URM,
            user_original_ID_to_index=self.user_original_ID_to_index,
            item_original_ID_to_index=self.item_original_ID_to_index,
        ...
        if items_to_remove is not None:
            ...
            if self._HAS_ICM:
                ...
                for ICM_name, ICM_object in self.AVAILABLE_ICM.items():
                    ...
                    ICM_object, _, ICM_mapper_object = remove_features(
                        ICM_object, min_occurrence=1, max_percentage_occurrence=1.00, reconcile_mapper=ICM_mapper_object
                    )
        ...
        if users_to_remove is not None:
            ...
            if self._HAS_UCM:
                ...
                for UCM_name, UCM_object in self.AVAILABLE_UCM.items():
                    ...
                    UCM_object, _, UCM_mapper_object = remove_features(
                        UCM_object, min_occurrence=1, max_percentage_occurrence=1.00, reconcile_mapper=UCM_mapper_object
                    )
