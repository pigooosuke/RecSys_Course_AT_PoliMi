...
class DataSplitter_Holdout(_DataSplitter):
    ...
    def __init__(
        self,
        dataReader_object: _DataReader,
        split_interaction_quota_list=None,
        user_wise=True,
        ...
        if split_interaction_quota_list is None:
            ...
            self._print(
                "input_split_item_quota_list not provided, using default '{}'".format(split_interaction_quota_list)
            )

        assert (
            len(split_interaction_quota_list) == 3
        ), "{}: input_split_item_quota_list must contain 3 values: Train, Validation, Test".format(
            self.DATA_SPLITTER_NAME
        )
        assert all(
            split_quota >= 0.0 and split_quota <= 100 for split_quota in split_interaction_quota_list
        ), "{}: input_split_item_quota_list must contain values between 0 and 100".format(self.DATA_SPLITTER_NAME)
        assert (
            sum(split_interaction_quota_list) == 100
        ), "{}: input_split_item_quota_list must be a probability distribution and sum to 100, current data sums to '{}'".format(
        ...
        super(DataSplitter_Holdout, self).__init__(
            dataReader_object, forbid_new_split=forbid_new_split, force_new_split=force_new_split
        )
    ...
    def get_statistics_URM(self):
        ...
        statistics_string = (
            ...
            "\tTest \t\tquota {:.2f} ({:.2f}), \tinteractions {}, \tdensity {:.2E}\n".format(
                ...
                n_items,
                n_users,
                self.input_split_interaction_quota_list[0],
                self.actual_split_interaction_quota_list[0],
                self.SPLIT_URM_DICT["URM_train"].nnz,
                compute_density(self.SPLIT_URM_DICT["URM_train"]),
                self.input_split_interaction_quota_list[1],
                self.actual_split_interaction_quota_list[1],
                self.SPLIT_URM_DICT["URM_validation"].nnz,
                compute_density(self.SPLIT_URM_DICT["URM_validation"]),
                self.input_split_interaction_quota_list[2],
                self.actual_split_interaction_quota_list[2],
        ...
        if len(self.dataReader_object.get_loaded_ICM_names()) > 0:
            ...
            for ICM_name, ICM_object in self.SPLIT_ICM_DICT.items():
                ...
                statistics_string = "\tICM name: {}, Num features: {}, feature occurrences: {}, density {:.2E}".format(
                    ICM_name, n_features, ICM_object.nnz, compute_density(ICM_object)
                )
    ...
    def _assert_is_initialized(self):
        assert (
            self.SPLIT_URM_DICT is not None
        ), "{}: Unable to load data split. The split has not been generated yet, call the load_data function to do so.".format(
            self.DATA_SPLITTER_NAME
        )
    ...
    def get_holdout_split(self):
        ...
        return (
            self.SPLIT_URM_DICT["URM_train"].copy(),
    ...
    def _save_split(self, save_folder_path):
        ...
        if save_folder_path:
            ...
            if len(self.SPLIT_ICM_DICT) > 0:
                ...
                dataIO.save_data(
                    data_dict_to_save=self.SPLIT_ICM_MAPPER_DICT, file_name="split_ICM_mappers" + name_suffix
                )
            ...
            if len(self.SPLIT_UCM_DICT) > 0:
                ...
                dataIO.save_data(
                    data_dict_to_save=self.SPLIT_UCM_MAPPER_DICT, file_name="split_UCM_mappers" + name_suffix
                )
    ...
    def _verify_data_consistency(self):
        ...
        assert len(self.SPLIT_URM_DICT) == len(self._SPLIT_URM_NAME_LIST), (
            print_preamble
            + "The available URM are not as many as they are supposed to be. URMs are {}, expected URMs are {}".format(
                len(self.SPLIT_URM_DICT), len(self._SPLIT_URM_NAME_LIST)
            )
        )

        assert all(URM_name in self.SPLIT_URM_DICT for URM_name in self._SPLIT_URM_NAME_LIST), (
        ...
        assert (
            self.SPLIT_URM_DICT["URM_validation"].nnz == 0 and self.input_split_interaction_quota_list[1] == 0.0
        ...
        for URM_index, URM_name in enumerate(self._SPLIT_URM_NAME_LIST):
            ...
            if actual_quota < min_value_allowed or actual_quota > max_value_allowed:
                print(
                    print_preamble
                    + "The differentce between the input interaction quota '{}' and actual interaction quota '{}' of '{}' higher than {} %".format(
        ...
        if not self.allow_cold_users:
            assert np.all(user_interactions != 0), (
                print_preamble
                + "Cold users exist despite not being allowed as per DataSplitter parameters, {} users out of {}".format(
