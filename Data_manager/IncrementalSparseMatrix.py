...
class IncrementalSparseMatrix_ListBased(object):
    ...
    def add_data_lists(self, row_list_to_add, col_list_to_add, data_list_to_add):
        ...
        assert len(row_list_to_add) == len(col_list_to_add) and len(row_list_to_add) == len(
            data_list_to_add
...
class IncrementalSparseMatrix(IncrementalSparseMatrix_ListBased):
    def __init__(
        self, auto_create_col_mapper=False, auto_create_row_mapper=False, n_rows=None, n_cols=None, dtype=np.float64
    ):

        super(IncrementalSparseMatrix, self).__init__(
            auto_create_col_mapper=auto_create_col_mapper,
    ...
    def get_SparseMatrix(self):
        ...
        sparseMatrix = sps.csr_matrix(
            (
                self._data_array[: self._next_cell_pointer],
...
class IncrementalSparseMatrix_FilterIDs(IncrementalSparseMatrix):
    ...
    def __init__(
        ...
        assert on_new_col in [
            "add",
            "ignore",
        ], "IncrementalSparseMatrix: if_new_col value not recognized, allowed values are 'add', 'ignore', provided was '{}'".format(
            on_new_col
        )
        assert on_new_row in [
            "add",
            "ignore",
        ], "IncrementalSparseMatrix: if_new_row value not recognized, allowed values are 'add', 'ignore', provided was '{}'".format(
            on_new_row
        )

        if on_new_col == "add":
            assert preinitialized_col_mapper is None or isinstance(
                preinitialized_col_mapper, dict
            ), "IncrementalSparseMatrix: if on_new_col is 'add' then preinitialized_col_mapper must be either 'None' or contain a dictionary"

        if on_new_row == "add":
            assert preinitialized_row_mapper is None or isinstance(
                preinitialized_row_mapper, dict
