...
class BaseRecommender(object):
    ...
    def __init__(self, URM_train, verbose=True):
        ...
        if self._cold_user_mask.any():
            self._print(
                "URM Detected {} ({:4.1f}%) users with no interactions.".format(
        ...
        if self._cold_item_mask.any():
            self._print(
                "URM Detected {} ({:4.1f}%) items with no interactions.".format(
    ...
    def set_URM_train(self, URM_train_new, **kwargs):
        ...
        assert (
            self.URM_train.shape == URM_train_new.shape
