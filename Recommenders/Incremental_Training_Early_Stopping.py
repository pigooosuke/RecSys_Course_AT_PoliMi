...
class Incremental_Training_Early_Stopping(object):
    ...
    def _train_with_early_stopping(
        ...
        ), "{}: epochs_min must be <= epochs_max, passed are epochs_min {}, epochs_max {}".format(
        ...
        assert (
            evaluator_object is None
            or (
                evaluator_object is not None
                and not stop_on_validation
                and validation_every_n is not None
                and validation_metric is not None
            )
            or (
                evaluator_object is not None
                and stop_on_validation
                and validation_every_n is not None
                and validation_metric is not None
                and lower_validations_allowed is not None
            )
        ...
        while epochs_current < epochs_max and not convergence:
            ...
            # Determine whether a validaton step is required
                ...
                if (
                    stop_on_validation
                    and lower_validatons_count >= lower_validations_allowed
                    and epochs_current >= epochs_min
                ):
                    convergence = True
            ...
            print(
                "{}: Epoch {} of {}. Elapsed time {:.2f} {}".format(
        ...
        if not convergence:
            ...
            if evaluator_object is not None and self.best_validation_metric is not None:
                ...
                print(
                    "{}: Terminating at epoch {}. Elapsed time {:.2f} {}".format(
