...
from Recommenders.MatrixFactorization.PyTorch.MF_MSE_PyTorch_model import DatasetIterator_URM, MF_MSE_PyTorch_model
...
class MF_MSE_PyTorch(BaseMatrixFactorizationRecommender, Incremental_Training_Early_Stopping):
    ...
    def fit(
        self, epochs=30, batch_size=128, num_factors=100, learning_rate=0.0001, use_cuda=True, **earlystopping_kwargs
    ):
    ...
    def _run_epoch(self, num_epoch):
        ...
        for num_batch, (input_data, label) in enumerate(self.train_data_loader, 0):
            ...
            if (num_batch + 1) % 10000 == 0 or (num_batch + 1) == len(self.train_data_loader):
                self._print(
                    "Epoch {}, Batch: [{}/{}], Samples per second {:.2f}".format(
                        num_epoch + 1,
                        num_batch + 1,
                        len(self.train_data_loader),
                        (num_batch + 1) * self.batch_size / (time.time() - start_time),
                    )
                )
